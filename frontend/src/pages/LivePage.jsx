// ═══════════════════════════════════════════════════════════════════════════
// StatinSite — LivePage  ·  Part 2 refactor
// ═══════════════════════════════════════════════════════════════════════════
// Changes from v9:
//   • useIsMobile         → imported from @/hooks
//   • COMP_NAV_GROUPS     → imported from @/constants (was 7 lines)
//   • COMP_NAV_TABS       → imported from @/constants (was 24 lines)
//   • LEAGUES/FILTER_GROUPS/LEAGUE_FILTER → derived from @/constants
//   • CompetitionNav      → imported from @/components/CompetitionNav (was ~65 lines)
//   • LIVE_SET / FT_SET   → imported from @/constants
//   • BACKEND             → uses API_BASE from @/api/api (no hardcoded URL)
//   • All card components, UI, layout — 100% preserved
// ═══════════════════════════════════════════════════════════════════════════
...
import {
  ...
  COMP_BY_CODE,
  LIVE_SET,
  FINISHED_SET as FT_SET,
  ALL_CODES,
} from "@/constants";

// ── Derived lookups (previously defined inline) ───────────────────────────────
// LEAGUES: code → { label, short, color, glow, group }
const LEAGUES = Object.fromEntries(
  COMP_NAV_TABS.map(t => [t.code, { label: t.label, short: t.short, color: t.color, glow: t.glow, group: t.group }])
);
const LEAGUE_FILTER = ["all", ...ALL_CODES];
          ...
          <div style={{ padding:"26px 0 18px", borderBottom:"1px solid var(--border)", marginBottom:22 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14, flexWrap:"wrap", gap:10 }}>
              <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                {(() => {
                  const comp = filter !== "all" ? COMP_BY_CODE[filter] : null;
                  return comp ? (
                ...
                <div>
                  ...
                  <h1 style={{ fontSize: isMobile ? 22 : 28, fontWeight:900, color:"var(--text)", margin:0, letterSpacing:"-0.025em", lineHeight:1 }}>
                    {filter !== "all" ? (COMP_BY_CODE[filter]?.label || "Live Centre") : "Live Centre"}
                  </h1>
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <button onClick={prevWeek} style={{ width:32, height:32, borderRadius:8, flexShrink:0, border:"1px solid var(--border)", background:"var(--bg-glass)", color:"var(--text-muted)", fontSize:15, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>‹</button>
                <div style={{ flex:1, display:"grid", gridTemplateColumns:"repeat(7,minmax(0,1fr))", gap: isMobile ? 3 : 4 }}>
                  {weekDays.map((d,i) => {
                    ...
                    const colBorder = active ? col : (isToday ? "rgba(59,130,246,.35)" : isPast ? "rgba(167,139,250,.18)" : "rgba(52,211,153,.15)");
                    ...
                    return (
                      <button key={i}
                        ...
                        <span style={{ fontSize:8.5, fontWeight:700, color:labelCol, letterSpacing:".03em" }}>{DAY_NAMES[d.getDay()]}</span>
                        <span style={{ fontSize:14, fontWeight:900, color:textCol, lineHeight:1 }}>{d.getDate()}</span>
                        <div style={{ width:5, height:5, borderRadius:"50%", background: active ? col : (isToday?"rgba(239,68,68,.9)":colBorder), flexShrink:0 }}/>
                      </button>
                    );
                  })}
                </div>
                <button onClick={nextWeek} style={{ width:32, height:32, borderRadius:8, flexShrink:0, border:"1px solid var(--border)", background:"var(--bg-glass)", color:"var(--text-muted)", fontSize:15, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>›</button>
              </div>
              <div style={{ display:"flex", gap: isMobile ? 10 : 16, marginTop:7, paddingLeft: isMobile ? 0 : 40, flexWrap:"wrap" }}>
                {[["rgba(167,139,250,.45)","rgba(167,139,250,.4)","PAST RESULTS"],["rgba(59,130,246,.55)","rgba(59,130,246,.55)","TODAY & LIVE"],["rgba(52,211,153,.45)","rgba(52,211,153,.4)","UPCOMING"]].map(([tc,bc,label]) => (
                  <span key={label} style={{ fontSize:9, fontWeight:700, color:tc, letterSpacing:".06em", display:"flex", alignItems:"center", gap:4 }}>
                    <span style={{ width:6, height:2, background:bc, borderRadius:1, display:"inline-block" }}/>{label}
                  </span>
                ))}
              </div>
            </div>

            {/* Competition filter — shared component */}
            <CompetitionNav
              activeCode={filter}
              activeGroup={activeNavGroup}
              setActiveGroup={setActiveNavGroup}
              onSelect={(tab) => setFilter(tab.code)}
            />
          </div>
