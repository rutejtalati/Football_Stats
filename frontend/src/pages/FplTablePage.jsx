...
export default function FplTablePage() {
  ...
  return (
    ...
    <div className="page-shell" style={{ paddingBottom: isMobile ? 80 : 40 }}>
      <style>{`
        /* ── All FPL table styles are now driven by CSS vars in index.css ── */
        /* Only dynamic (JS-interpolated) values go here */
        .fpl-tbl { min-width:${isMobile ? "900px" : "1400px"}; }
        .sticky-player { min-width:${isMobile ? "140px" : "190px"}; }
        .team-badge-cell { font-size:11px; font-weight:800; text-align:center; padding:4px 6px !important; }
        .fpl-tbl th.col-sep, .fpl-tbl td.col-sep { border-left:1px solid var(--border-strong); }
        /* Light mode: col sep uses darker border */
        [data-theme="light"] .fpl-tbl th.col-sep,
        [data-theme="light"] .fpl-tbl td.col-sep { border-left:1px solid rgba(0,0,0,0.14); }
      `}</style>
      ...
      <div className="page-content-wide">
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
          ...
          {isMobile && (
            <button onClick={() => setShowFilters(v => !v)} style={{
              ...
              background:showFilters?"var(--pill-active-bg)":"var(--pill-bg)",
              border:"1px solid var(--pill-active-border)", color:"var(--pill-active-color)",
        ...
        {(showFilters || !isMobile) && (
          <div style={{ background:"var(--bg-glass)", border:"1px solid var(--border)",
            ...
            <div style={{ display:"flex", gap:5, flexWrap:"nowrap", overflowX:"auto",
              ...
              <div style={{ width:1, height:28, background:"var(--border-strong)", flexShrink:0, alignSelf:"center" }}/>
            ...
            <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr 1fr":"1fr 1fr 1fr 1fr auto", gap:8 }}>
              {[
                ...
                <div key={label} style={{ display:"flex", flexDirection:"column", gap:3 }}>
                  <label style={{ fontSize:9, fontWeight:800, color:"var(--text-muted)", letterSpacing:"0.08em" }}>{label}</label>
                  <input type={type} step={step} min={min} max={max} value={val}
                    onChange={e => set(Number(e.target.value))}
                    style={{ padding:"7px 10px", borderRadius:8, fontSize:13,
                      background:"var(--input-bg)", border:"1px solid var(--input-border)",
                      color:"var(--input-color)", outline:"none", minHeight:36 }}/>
                </div>
              ))}
              <div style={{ display:"flex", flexDirection:"column", gap:3, gridColumn:isMobile?"1/-1":"auto" }}>
                <label style={{ fontSize:9, fontWeight:800, color:"var(--text-muted)", letterSpacing:"0.08em" }}>SEARCH</label>
              ...
              <button onClick={() => { setTeam("ALL"); setPosition("ALL"); setMaxCost(15.5); setMinProb(0); setStartGw(30); setSearch(""); }}
                style={{ padding:"7px 14px", borderRadius:8, fontSize:11, fontWeight:800,
                  ...
                  color:"var(--text-muted)", cursor:"pointer", fontFamily:"inherit", alignSelf:"flex-end",
        ...
        {/* ── Table ── */}
        <div style={{ background:"var(--table-bg)", border:"1px solid var(--table-border)",
    ...
    {tooltip && (
      <div className="fpl-tip" style={{ left:tooltip.x, top:tooltip.y }}>
        <div style={{ fontSize:9, fontWeight:800, color:"var(--text-muted)", letterSpacing:"0.1em", marginBottom:6, textTransform:"uppercase" }}>
      ...
      <div className="fpl-modal-bg" onClick={() => setModal(null)}>
        <div className="fpl-modal" onClick={e => e.stopPropagation()}>
          <button onClick={() => setModal(null)} style={{
            ...
            background:"var(--bg-glass)", border:"1px solid var(--border)",
            color:"var(--text-muted)", cursor:"pointer", fontSize:16, display:"flex", alignItems:"center", justifyContent:"center",
          }}>×</button>

          {/* Header */}
          <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:20 }}>
            <div style={{ width:52, height:52, borderRadius:"50%", overflow:"hidden",
              border:`2px solid ${modal._posColor}33`, background:"var(--bg-glass)", flexShrink:0 }}>
              <img src={`https://resources.premierleague.com/premierleague/photos/players/110x140/p${modal.code||0}.png`}
                alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }}
                onError={e => { e.target.style.display="none"; }}/>
            </div>
            <div>
              <div style={{ fontSize:18, fontWeight:900, color:"var(--text)", fontFamily:"'Sora',sans-serif", marginBottom:2 }}>
                {modal.player}
              </div>
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                <span style={{ fontSize:10, fontWeight:800, color:"var(--text-muted)" }}>{modal.team}</span>
                ...
                <span style={{ fontSize:10, color:"var(--text-secondary)", fontFamily:"DM Mono,monospace" }}>£{modal.cost}m</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ marginBottom:16, padding:"8px 12px", borderRadius:8,
            ...
            {modal.news && <span style={{ fontSize:10, color:"var(--text-muted)", marginLeft:8 }}>{modal.news}</span>}
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:16 }}>
            {[
              ...
              { label:"Form",          val:(modal.form||0).toFixed(1),           color:"var(--amber)" },
              { label:"Captain Score", val:(modal.captain_score||0).toFixed(1), color:"var(--orange)" },
              { label:"Start %",       val:`${modal.avail_pct}%`,               color:modal.avail_color },
              { label:"Owned",         val:`${Number(modal.selected_by_pct||0).toFixed(1)}%`, color:"var(--purple)" },
            ].map(m => (
              <div key={m.label} style={{ padding:"10px 12px", borderRadius:10,
                background:"var(--bg-glass)", border:"1px solid var(--border)" }}>
                <div style={{ fontSize:8, fontWeight:800, color:"var(--text-muted)", letterSpacing:"0.1em", marginBottom:4, textTransform:"uppercase" }}>
                  {m.label}
                </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, marginBottom:16 }}>
            {[
              ...
              <div key={m.label} style={{ padding:"8px 10px", borderRadius:8,
                background:"var(--bg-glass)", border:"1px solid var(--border-soft)", textAlign:"center" }}>
                <div style={{ fontSize:8, fontWeight:700, color:"var(--text-muted)", marginBottom:3, textTransform:"uppercase" }}>{m.label}</div>
                <div style={{ fontSize:15, fontWeight:800, color:"var(--text)", fontFamily:"DM Mono,monospace" }}>{m.val ?? "—"}</div>
              </div>
            ))}
          </div>
          ...
          <button onClick={() => { setModal(null); navigate(`/player/${modal.player_id}`); }}
            style={{ width:"100%", padding:"10px 16px", borderRadius:10,
              background:"var(--blue-soft)", border:"1px solid var(--blue-glow)",
              color:"var(--blue)", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
